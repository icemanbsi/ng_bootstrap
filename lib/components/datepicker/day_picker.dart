...
class BsDayPickerComponent {
  ...
  ///
  List<DateTime> getDates(DateTime startDate, int n) {
    var dates = List<DateTime>.filled(n, DateTime.now());
  ...
  ///
  int getISO8601WeekNumber(DateTime checkDate) {
  ...
  void refreshViewHandler() {
    ...
    var difference = datePicker != null ? (datePicker!.startingDay - firstDayOfMonth.day) : 0;
...
//        firstDate.setDate(-numDisplayedFromPreviousMonth + 1);
    ...
    if (datePicker?.showWeeks ?? false) {
      weekNumbers.clear();
      int thursdayIndex = (4 + 7 - (datePicker?.startingDay ?? 0)) % 7, numWeeks = rows.length;
      for (int curWeek = 0; curWeek < numWeeks; curWeek++) {
