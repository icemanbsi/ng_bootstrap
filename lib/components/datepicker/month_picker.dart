...
class BsMonthPickerComponent {
  ...
  void refreshViewHandler() {
    var months = List<DisplayedDate>.filled(12, DisplayedDate(DateTime.now(), '', false, false, false));
    var initDate = datePicker?._initDate ?? DateTime.now();
    int year = initDate.year;
    DateTime date;
    for (var i = 0; i < 12; i++) {
      date = DateTime (year, i + 1, 1);
      months[i] = datePicker?.createDateObject(date, datePicker?.formatMonth ?? '') ?? DisplayedDate(DateTime.now(), '', false, false, false);
    }
